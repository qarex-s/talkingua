...
namespace TalkingUADev.Controllers
{
    public class UserEventController : Controller
    {
        ...
        public UserEventController(ApplicationDbContext context, UserManager<UserApp> user, IWebHostEnvironment webHostEnvironment)
        {
            _context = context;
            _user = user;
        ...
        public async Task<IActionResult> AddPublication(ModelPost post)
        {
            ...
            {
                UserApp someUser = await _user.GetUserAsync(User);
                ...
                if (post.ImageFile != null)
                {
                    ...
                    string SubDirPath = $"UserPostBy{someUser.Email}";

                    DirectoryInfo directoryInfo = new DirectoryInfo(wwwRootPath + "/UsersStaticFiles");
                    ...
                    post.ImageTitle = someUser.Email + fileName + post + extension;
                    string path = Path.Combine(wwwRootPath + "/UsersStaticFiles/" + SubDirPath + "/" + post.ImageTitle);
                    ...
                    tempUserPost.ImageFile = post.ImageFile;
        ...
        {
            var users = await _user.Users.Where(x => x.Name
            .Contains(UserName))
                .ToListAsync();
            if (users != null)
            {
        ...
        public async Task<IActionResult> ViewProfileUser(string userId)
        {
            ...
            if (UserGet != null)
            {
                ...
                var FollowerOrNot = await _context.followUsers
                    .Where(x => x.FollowerId == userId && x.UserId == _user.GetUserId(User))
                    .FirstOrDefaultAsync();
        ...
        public async Task<IActionResult> FollowToUser(string userId)
        {
            ...
            MainUser.CountFollows = _context.followUsers
                .Where(_x => _x.UserId == MainUser.Id && _x.isFollowed == true)
                .Count();
        ...
        {
            var LikeUserTemp = await _context.likesUsers
                .Where(x => x.PostId == postId && x.UserId == _user.GetUserId(User))
                .FirstOrDefaultAsync();
        ...
        public async Task<IActionResult> ViewLikedPost()
        {
            ...
            var user = await _user.GetUserAsync(User);
            var postsId = await _context.likesUsers
                .Where(x => x.UserId == user.Id && x.isLiked).Select(x => x.PostId)
                .ToListAsync();

            return View(await _context.Posts
                .Where(x => postsId
                .Contains(x.UserPostId.ToString()))
                .Include(x => x.user)
                .OrderByDescending(x => x.DateOfCreatingPost)
                .ToListAsync());
        }
